...
def clear_failed_for_courses(courses: dict) -> None:
    """Belirtilen kursların başarısız item'larını siler (yeniden deneme için)."""
    if not PROGRESS_FILE.exists():
        return
    ...
    items = {
        ...
        if not (k in all_ids and v.get("status") == "failed")
    }
